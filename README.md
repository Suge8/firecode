...
也折进去，提示条数在摘要行标为 `⚠ N`；段尾的正式回复、错误消息显示在摘要下方。流式中最新的一段正文可见，后面一接
工具就收进摘要。摘要行左侧是当前动作与目标（进行中 `✦`，完成 `✓`，有失败 `✗` 并标 `N 次失败`），右侧按
工具类别计数如 `读取 5 · 操作 2 · 修改 1`；终端变窄时先整列丢掉计数，再裁目标，动作与失败标记始终保留。
不统计思考次数或耗时。
