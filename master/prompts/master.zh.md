...
- 派发：role 只使用调研员、工程师、全栈、架构师、设计师、哨兵六个固定词；start 按已配置角色表显式传 role，send 需要换职能时也传 role；thinking 仅在用户明确要求时覆盖角色原子档。供应商额度或计费耗尽、模型不可用、宿主重试用尽的持续 5xx 会自动按该角色 fallback 降级并回报。以等待为主的盯守（CI、部署、长测试）使用哨兵。
