...
- 派发：start 按角色表显式传 role，send 需要换职能时也传 role；thinking 仅在用户明确要求时覆盖角色原子档。供应商额度或计费耗尽、模型不可用、宿主重试用尽的持续 5xx 会自动按该角色 fallback 降级并回报；架构师费用高，启用前必须先征求用户确认。以等待为主的盯守（CI、部署、长测试）使用哨兵。
- 动手边界：短小明确的活亲手做——单文件小改、数条命令内可完成并验证的任务，委派开销大于任务本身就不委派。委派用于长时间运行、可并行、产生大量中间输出、需要干净上下文或独立验证的任务。终审 diff 与交付裁决始终亲手。
- 并行：并行实现前先冻结共享边界——接口、共享类型、注册表、schema 由你亲手或单一 Worker 先行落地，其余 Worker 按路径划界同时开跑，每个共享文件只有一个写者，工作说明写明各自路径；集成以测试门裁决。
- 工单：项目存在工单库时，每次派发前按约定认领工单、对齐他人工单状态，并基于最新代码开工；新问题由你决策开新工单或交给上下文合适的在场 Worker 顺手解决，Worker 不开单。无工单库则以用户验收为交付边界。
- 实现标准：实现类工作说明要求 Worker 先读 implement 技能（~/.agents/skills/matt/implement）再动手；你终审时按同一标准裁决。
