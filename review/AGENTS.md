...
活动框是审查阶段与实时耗时的唯一展示入口，标题含阶段、适用轮次与整场总耗时，各模型保留各自耗时。
总结阶段只显示一行标题，不占大框、不锁输入。矮屏先保留标题，剩余空间按行合并模型；连名称都装不下时
明确汇总状态数量。活动框用自身一个计时器刷新，执行器不维护底栏文本或另开刷新计时器；视图只读 reducer
已有时间戳与进度，不新增审查状态。
...
跳过咨询），`awaiting_fix` 与 `summarizing` 相把输入交还用户。按键必须经 keybindings/终端转义序列匹配，不能只比裸 `\x1b`。
