...
审查意图在 `start`/`send` 的 `review:true` 参数声明（跟任务走不跟渠道走，唤醒待命不设）、持久化进 Worker 档案；意图只在 review 确认启动
（状态变化或 runId 推进）时消耗——投递失败保留意图并通知，reload/reconcile 与休眠恢复都凭档案续上补审；
投递窗口内的审查票拒绝 send（blocked 提问的回答通道不受影响；中断态不是投递窗口，审查票的 send 在中断态放行以保自动续跑可执行）。Worker 成功落定即自动发起补审并回传终态
